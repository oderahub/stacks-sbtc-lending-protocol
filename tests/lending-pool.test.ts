...
describe('Lending Pool Tests', () => {
  ...
  it('Borrower can borrow and repay STX, lender earns yield', () => {
    ...
    const { result: repayResult } = simnet.callPublicFn('lending-pool', 'repay', [], borrower)
  ...
  it('Borrower can borrow, gets liquidated if price drops below liquidation threshold', () => {
    ...
    expect(withdrawResult).toBeOk(Cl.bool(true))
  }, 120_000) // 120 second timeout for heavy DEX swap + interest accrual
})
...
function getTotalCollateral() {
  const totalCollateral = simnet.getDataVar('lending-pool', 'total-sbtc-collateral')
  return Number(cvToValue(totalCollateral))
}

function getUserDebt(user: string) {
  const { result } = simnet.callReadOnlyFn('lending-pool', 'get-debt', [Cl.principal(user)], user)
...
function getPendingYield(user: string) {
  const { result } = simnet.callReadOnlyFn('lending-pool', 'get-pending-yield', [], user)
...
function updateOracle(price: number) {
  const { result } = simnet.callPublicFn('mock-oracle', 'update-price', [Cl.uint(price)], updater)
  expect(result).toBeOk(Cl.bool(true))
}
