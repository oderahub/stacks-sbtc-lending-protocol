...
describe('Mock Oracle Tests', () => {
  it('should initialize the oracle with the correct owner and updater', () => {
    ...
    const storedUpdater = simnet.callReadOnlyFn('mock-oracle', 'get-updater', [], deployer)
    expect(storedUpdater.result).toEqual(Cl.principal(updater))

    const isInitialized = simnet.callReadOnlyFn('mock-oracle', 'is-initialized', [], deployer)
  ...
  it('should not allow re-initialization', () => {
    simnet.callPublicFn('mock-oracle', 'initialize', [Cl.principal(updater)], deployer)
    const { result } = simnet.callPublicFn(
      'mock-oracle',
      'initialize',
      [Cl.principal(updater)],
  ...
  it('should allow updater to update the price', () => {
    simnet.callPublicFn('mock-oracle', 'initialize', [Cl.principal(updater)], deployer)
    const { result } = simnet.callPublicFn('mock-oracle', 'update-price', [Cl.uint(50000)], updater)
    expect(result).toBeOk(Cl.bool(true))

    const price = simnet.callReadOnlyFn('mock-oracle', 'get-price', [], deployer)
  ...
  it('should get the correct price', () => {
    simnet.callPublicFn('mock-oracle', 'initialize', [Cl.principal(updater)], deployer)
    simnet.callPublicFn('mock-oracle', 'update-price', [Cl.uint(50000)], updater)
    const { result } = simnet.callReadOnlyFn('mock-oracle', 'get-price', [], deployer)
    expect(result).toEqual(Cl.ok(Cl.uint(50000)))
  })

  it('should get the correct updater', () => {
    simnet.callPublicFn('mock-oracle', 'initialize', [Cl.principal(updater)], deployer)
    const { result } = simnet.callReadOnlyFn('mock-oracle', 'get-updater', [], deployer)
    expect(result).toEqual(Cl.principal(updater))
  })

  it('should get the correct initialized status', () => {
    const { result: before } = simnet.callReadOnlyFn('mock-oracle', 'is-initialized', [], deployer)
    expect(before).toEqual(Cl.bool(false))

    simnet.callPublicFn('mock-oracle', 'initialize', [Cl.principal(updater)], deployer)

    const { result: after } = simnet.callReadOnlyFn('mock-oracle', 'is-initialized', [], deployer)
